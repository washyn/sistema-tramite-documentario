...
function abrirModalExportar() {
	...
	$('#txt_fecha_inicio').data("datetimepicker").date(fecha);
	$('#txt_fecha_fin').data("datetimepicker").date(fecha);
	$("#combo_estado_reporte").val("%").trigger("change");  
	$('#modal_reporte_tramite').modal({backdrop: 'static', keyboard: false})
    $("#modal_reporte_tramite").modal("show");
}
function reportar_pdf(argument) {
	var txt_fechaincio = $("#txt_fechaincio").val();  
	var txt_fechafin   = $("#txt_fechafin").val();  
	var estado         = $("#combo_estado_reporte").val();  
	if (txt_fechaincio.length==0 ) {
		return Swal.fire("Falta Seleccionar fecha inicio","","warning");
	}
	if ( txt_fechafin.length==0) {
		return Swal.fire("Falta Seleccionar fecha fin","","warning");
	}
	var inicio         = txt_fechaincio.split('/');
	var fecnainicio    = inicio[2] + '-' + inicio[1] + '-' + inicio[0];
	var fin            = txt_fechafin.split('/');
	var fecnafin       = fin[2] + '-' + fin[1] + '-' + fin[0];
	window.open("MPDF/REPORTES/generar_pdf_tramites_general.php?fecnainicio="+fecnainicio+"&fecnafin="+fecnafin+"&estado="+estado+"#zoom=100"); 
}
	...
	if (u.length == 0 || p.length == 0) {
		...
		.done(function(resp){
			if (resp==0) {
				...
				if (data[0][18]=="INACTIVO"){
					return Swal.fire("Mensajde de advertencia","Lo sentimos el &aacute;rea de su cuenta "+u+" esta suspendida"+"Para mas informacion comuniquese con su administrador","warning");
				}
				$.ajax({
